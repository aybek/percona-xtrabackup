...
dberr_t Tablespace_dirs::scan(bool populate_fil_cache, bool is_prep_handle_ddls) {
  ...
  if (err == DB_SUCCESS && populate_fil_cache) {
    bool result = true;
    std::function<void(const Const_iter &, const Const_iter &, size_t, bool &, bool)>
        open = std::bind(&Tablespace_dirs::open_ibd, this, _1, _2, _3, _4, _5);

    par_for(PFS_NOT_INSTRUMENTED, ibd_files, n_threads, open, result, is_prep_handle_ddls);
...
@return DB_SUCCESS if all goes well */
dberr_t fil_scan_for_tablespaces(bool populate_fil_cache, bool is_prep_handle_ddls) {
